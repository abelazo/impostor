...
describe("ImpostorCounter", () => {
  describe("display", () => {
    it("shows impostor count selector", () => {
      render(
        <ImpostorCounter participantCount={4} value={1} onChange={vi.fn()} />,
      );
  ...
  describe("increment/decrement", () => {
    it("increments impostor count when clicking +", async () => {
      ...
      render(
        <ImpostorCounter participantCount={6} value={1} onChange={onChange} />,
      );
    ...
    it("decrements impostor count when clicking -", async () => {
      ...
      const onChange = vi.fn();
      render(
        <ImpostorCounter participantCount={6} value={2} onChange={onChange} />,
      );
  ...
  describe("minimum constraint", () => {
    ...
    it("does not allow going below 1", async () => {
      ...
      const onChange = vi.fn();
      render(
        <ImpostorCounter participantCount={4} value={1} onChange={onChange} />,
      );
  ...
  describe("maximum constraint (participants/2, min 1)", () => {
    it("disables increase button at maximum for 4 participants (max 1)", () => {
      // max = floor(4/2) = 2
      render(
        <ImpostorCounter participantCount={4} value={2} onChange={vi.fn()} />,
      );
    ...
    it("disables increase button at maximum for 6 participants (max 2)", () => {
      // max = floor(6/2) = 3
      render(
        <ImpostorCounter participantCount={6} value={3} onChange={vi.fn()} />,
      );
      expect(screen.getByRole("button", { name: /increase/i })).toBeDisabled();
    });
    ...
    it("disables increase button at maximum for 10 participants (max 4)", () => {
      // max = floor(10/2) = 5
      render(
        <ImpostorCounter participantCount={10} value={5} onChange={vi.fn()} />,
      );
      expect(screen.getByRole("button", { name: /increase/i })).toBeDisabled();
    });
    ...
    it("allows increase when below maximum", () => {
      render(
        <ImpostorCounter participantCount={10} value={2} onChange={vi.fn()} />,
      );
    ...
    it("ensures minimum of 1 for max with 2 participants", () => {
      // max = floor(2/2) = 1
      render(
        <ImpostorCounter participantCount={2} value={1} onChange={vi.fn()} />,
      );
